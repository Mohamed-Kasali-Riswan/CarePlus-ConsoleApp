...
public class ManageRecepView extends BaseView implements IManageRecepView {

	private final IManageRecepPresenter presenter;

	public ManageRecepView() {
		presenter = new ManageRecepPresenter(this);
	}
	...
	public void proceedFeatures() {
			...
			while (flag) {
				...
				try {
					...
					case 1: {
						presenter.addReceptionist(getReceptionist());
						break;
					}
					case 2: {
						String recId = getReceptionistId();
						if (presenter.containsReceptionistById(recId))
							presenter.removeReceptionist(recId);
						else
							showMessage("There is no Receptionist with this ID");
						break;
					}
					case 3: {
						presenter.viewAllReceptionist();
						break;
					}
					case 4: {
						flag = false;
	...
	private String getReceptionistId() {
		...
		do {
			...
			if (!recId.startsWith("RECEP"))
				System.out.print("Invalid Receptionist ID! re-enter:");
			else
				return recId;
		} while (true);
	}
	...
	public String getUserName() {
		...
		do {
			...
			if (uname.length() < 5 || uname.length() > 20)
				System.out.print("Invalid Username! re-enter:");
			else
				return uname;
		} while (true);
	}
